...
MODEL_MAP = {'bert-base': 'dbmdz/bert-base-turkish-cased'}
...
class RerankDataset(Dataset):
    ...
    def load_data(self):
        ...
        for s_idx, (step, info) in enumerate(d.items()):
            ...
            if self.args.add_goal: # rehber başlığı ekle
...
def eval(data, model, args, return_prediction=False, pos_label=True):
    ...
    bs_res = []
    for step, data in enumerate(tqdm(data, desc='eval')):
        try:
            assert args.bs == 1
...
def run(args):
    ...
    for ep in range(args.epochs):
        ...
        if ep >= args.min_save_ep:
            ...
            utils.save_model(metric, model, optimizer, scheduler, args, args.save_path.replace("epoch", f"ep{ep}"))
...
def config():
    ...
    args.per_sample_tot = args.neg_num + 1 # örnek başına düşen cümle sayısı
